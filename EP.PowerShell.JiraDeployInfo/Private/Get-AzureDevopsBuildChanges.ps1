function Get-AzureDevopsBuildChanges {
    ...
    Write-Verbose("Build changes (artificatDatda.data)" + ($response.fps.dataProviders.data.'ms.vss-traceability-web.traceability-run-changes-data-provider'.artifactsData[0].artifactVersion.id | ConvertTo-Json -Depth 100))

    $build_changes = $response.fps.dataProviders.data.'ms.vss-traceability-web.traceability-run-changes-data-provider'.artifactsData[0].artifactVersion | ForEach-Object { $_.message }
